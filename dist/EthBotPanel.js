...
export function EthBotPanel({ vaultAddress, chainId = mainnet.id, className, title = "EtherTrade Bot", onSubmitted }) {
    ...
    return (_jsxs("section", { className: className, style: {
            ...
            maxWidth: 520,
            padding: 18
        }, children: [_jsxs("header", { style: { display: "grid", gap: 6 }, children: [_jsx("h2", { style: { fontSize: 18, lineHeight: 1.2, margin: 0 }, children: title }), _jsxs("div", { style: { color: "#4b5563", fontSize: 14 }, children: ["Status: ", status] })] }), _jsxs("div", { style: { display: "grid", gap: 8 }, children: [_jsx(Row, { label: "ETH / USDT", value: price.priceText }), _jsx(Row, { label: "24h change", value: price.changePercent24hText }), _jsx(Row, { label: "Ticker", value: `${price.source} ${price.status}` }), _jsx(Row, { label: "Wallet ETH", value: `${Number(wallet.balanceEth).toFixed(6)} ETH` }), _jsx(Row, { label: "Funded ETH", value: `${Number(vault.balanceEth).toFixed(6)} ETH` }), _jsx(Row, { label: "Funded value", value: formatUsd(bot.fundedUsd) })] }), _jsxs("label", { style: { display: "grid", gap: 6, fontSize: 14, fontWeight: 700 }, children: ["Fund amount ETH", _jsx("input", { inputMode: "decimal", min: "0", onChange: (event) => setFundAmountEth(event.target.value), placeholder: "0.01", step: "any", style: {
                            border: "1px solid #d1d5db",
                            borderRadius: 6,
                            font: "inherit",
                            minHeight: 42,
                            padding: "0 12px"
                        }, type: "number", value: fundAmountEth })] }), _jsxs("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }, children: [_jsx("button", { disabled: !canUseVault || vault.depositsPaused, onClick: () => void runAction("deposit", () => vault.depositEth(fundAmountEth)), style: { ...buttonStyle, opacity: canUseVault && !vault.depositsPaused ? 1 : 0.5 }, type: "button", children: "Fund ETH" }), _jsx("button", { disabled: !canUseVault || !bot.isFunded, onClick: () => void runAction(bot.isRunning ? "stopBot" : "startBot", bot.isRunning ? vault.stopBot : vault.startBot), style: { ...buttonStyle, opacity: canUseVault && bot.isFunded ? 1 : 0.5 }, type: "button", children: bot.isRunning ? "Stop Bot" : "Start Bot" })] }), _jsx("button", { disabled: !canUseVault || vault.balanceWei === 0n, onClick: () => void runAction("withdrawAll", vault.withdrawAll), style: { ...secondaryButtonStyle, opacity: canUseVault && vault.balanceWei > 0n ? 1 : 0.5 }, type: "button", children: "Withdraw ETH" }), vault.pendingHash ? (_jsx("a", { href: `https://etherscan.io/tx/${vault.pendingHash}`, rel: "noreferrer", style: { color: "#2563eb", fontSize: 14, overflowWrap: "anywhere" }, target: "_blank", children: "View transaction" })) : null, localError || vault.error || price.error || wallet.error ? (_jsx("div", { style: { color: "#b91c1c", fontSize: 14 }, children: localError ?? vault.error?.message ?? price.error ?? wallet.error?.message })) : null] }));
}
