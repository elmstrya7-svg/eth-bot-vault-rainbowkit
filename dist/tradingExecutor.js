import { createPublicClient, createWalletClient, custom, encodeFunctionData, formatEther, maxUint256, parseAbi } from "viem";
...
function applySlippage(value, slippageBps) {
    return (value * BigInt(10_000 - slippageBps)) / 10000n;
}
function createClients(provider, account) {
...
export async function previewUniswapV2RoundTrip(options) {
    ...
    if (!approvedRouter)
        throw new Error("Router is not approved.");
    if (!approvedBuySelector || !approvedSellSelector)
        throw new Error("Router selectors are not approved.");
    ...
    if (executorBalance < options.tradeValue)
        throw new Error("Executor balance is lower than tradeValue.");
...
export async function approveUniswapV2RoundTripToken(options) {
    const routerAddress = options.routerAddress ?? UNISWAP_V2_ROUTER;
    ...
    if (allowance >= options.tradeValue)
        return undefined;
...
//# sourceMappingURL=tradingExecutor.js.map